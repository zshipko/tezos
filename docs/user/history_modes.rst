...
See :ref:`Perserving additional cycles<History mode additional cycles>` for
details.

History modes comes with some checkpoints which are used to describe
the state of the storage:

- `checkpoint`: the last allowed fork level of the chain,
- `savepoint`: the last known block which contains metedata,
...
In full mode, the `savepoint` is the oldest block which contains its
metadata. The caboose is the oldest known block which is pruned (that
...
.. _History mode additional cycles:
...
window. Depending on the network, a number of cycles are initially
preserved (e.g., 7 on mainnet). In order to increase this number, it
is possible to configure the node to keep additional cycles.

By default, the number of preserved additional cycles, for
both ``full`` and ``rolling`` nodes, is *5 cycles*. On mainnet, this
would total *12 cycles* of complete history (approximately a month).
It is possible to increase this parameter to increase this limit or,
on the contrary, decrease it to reduce the storage size. For example,
it is possible to run a baker and a delegation service on rolling mode
with *7 additional cycles* providing two more weeks to dispatch
rewards.
...
restrictions<Switch mode restrictions>`
...
It is possible to switch between history modes and the number of
additional cycles. To do so, it is necessary to restart the node with
the desired history mode and add the flag
