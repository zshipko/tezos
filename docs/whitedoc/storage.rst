...
their associated metadata which consists of operation receipts). It is
composed of two main parts: the :ref:`store<store_component>` and the
...
store also handle the chain's current state: current head, invalid
blocks, active test chains, etc. The store component is designed to handle
concurrent accesses to the data. Both a mutex and a lockfile are
present to prevent concurrent access to critical sections. The store
also manages the :ref:`context<context_component>` and handles its
initialization, but it is not responsible to commit contexts
...
- The *savepoint* which indicates the lowest block known by the store
  that possess metadata.

The checkpoint is also a special value which indicates the block that
...
Persistent protocols are also written on-disk.
...
both context hashes are different, the block is considered invalid.
