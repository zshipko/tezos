...
class TestMultiNodeStorageReconstruction:
    ...
    def test_reconstruct_on_bootstrapped_node(self, sandbox):
    ...
    def test_export_snapshot_batch1(self, sandbox, session):
    ...
    def test_node1_import_and_reconstruct(self, sandbox, session):
    ...
    # with their metadata
    def test_node1_request_all_blocks_with_metadata(self, sandbox, session):
    ...
    def test_import_before_reconstruct(self, sandbox, session):
        n0_tmpdir = sandbox.node(0).node_dir
        file = f'{n0_tmpdir}/{SNAPSHOT_1}'
    ...
    def test_unavailable_blocks_node2(self, sandbox, session):
    ...
    def test_reconstruct_after_snapshot_import(self, sandbox):
        # Stop, reconstruct the storage and restart the node
    ...
    # Test that all the reconstructed blocks can be requested
    # with their metadata
    def test_available_blocks_node_2(self, sandbox, session):
    ...
    def test_export_snapshot_batch2(self, sandbox, session):
        node_export = sandbox.node(0)
    ...
    def test_reconstruct_command_after_bootstrap(self, sandbox):
        # Stop, reconstruct the storage and restart the node
        sandbox.node(3).terminate_or_kill()
        sandbox.node(3).reconstruct()
    ...
    def test_available_blocks_node3(self, sandbox, session):
